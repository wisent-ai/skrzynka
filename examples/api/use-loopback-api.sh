...
: "${WISENT_ORGANIZATION_ID:?WISENT_ORGANIZATION_ID is required}"
umask 077
AUTH_HEADERS=$(mktemp "${TMPDIR:-/tmp}/skrzynka-api-headers.XXXXXX")
trap 'rm -f "$AUTH_HEADERS"' EXIT HUP INT TERM
...
curl --fail --silent --show-error --header "@$AUTH_HEADERS" \
  --request POST "$API_URL/v1/mailboxes/$MAILBOX_ID/sync"
printf '\n'
printf '%s\n' '== observe: read its bounded normalized inbox'
curl --fail --silent --show-error --header "@$AUTH_HEADERS" \
  "$API_URL/v1/messages?mailbox_id=$MAILBOX_ID&limit=20"
printf '\n'

printf '%s\n' 'Expected result: a ready health response, one mailbox UUID, and messages when provider INBOX contains mail.'
printf '%s\n' "Failure: authenticated GET $API_URL/v1/mailboxes/$MAILBOX_ID exposes the normalized mailbox error without credentials."
printf '%s\n' "Cleanup: authenticated DELETE $API_URL/v1/mailboxes/$MAILBOX_ID?confirm=true"
